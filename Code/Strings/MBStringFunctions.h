...
static inline NSString* MBForceString(id x) { return (x ? (x == [NSNull null] ? @"" : [x description]) : @""); }

//!< given any object, forces it to a string and trims whitespace from the ends
static inline NSString* MBTrimString(id x) { return (x ? (x == [NSNull null] ? @"" : [[x description] stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]]) : @""); }
