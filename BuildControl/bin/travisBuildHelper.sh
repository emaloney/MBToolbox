...
while [[ $THIS_TRY < $MAXIMUM_TRIES ]]; do
	...
	( set -o pipefail && xcodebuild -project MBToolbox.xcodeproj -configuration Debug -scheme "MBToolbox" -destination "$DESTINATION" -destination-timeout 300 $XCODE_ACTION 2>&1 | tee "MBToolbox-$PLATFORM-$OPERATION.log" | xcpretty )
